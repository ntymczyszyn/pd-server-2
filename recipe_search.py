...
def setup_tfidf(data_path):
    ...
    print("[TF-IDF] Setup done.")
    return df, vectorizer, tfidf_matrix


def find_recipe_tfidf(search_terms, df, vectorizer, tfidf_matrix, max_recipes):
    results = pd.DataFrame()
    search_terms = [
        term.replace("_", " ") if "_" in term else term for term in search_terms
    ]
    ...
    for r in range(len(search_terms), 0, -1):
        ...
        for combination in term_combinations:
            ...
            filtered_indices = cosine_similarities >= 0.3
            filtered_df = df.loc[filtered_indices].copy()

            def full_match(ner_list, combination):
                return all(
                    any(term.lower() in ingredient.lower() for ingredient in ner_list)
            ...
            filtered_df["MatchedCount"] = filtered_df["NERList"].apply(
                lambda ner_list: sum(
                    ...
                    for term in combination
                    if any(
                        term.lower() in ingredient.lower() for ingredient in ner_list
                    )
                )
            )

            filtered_df["FullMatch"] = filtered_df["NERList"].apply(
                lambda ner_list: full_match(ner_list, combination)
            )
    ...
    unique_results = results.head(max_recipes)
